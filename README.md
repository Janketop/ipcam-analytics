...
- `YOLO_FACE_MODEL` — путь до весов детектора лиц (по умолчанию `weights/yolo11n-face.pt`). Файл можно получить двумя способами: (1) скачать готовые веса, сервис попробует найти их на GitHub/HuggingFace автоматически; (2) обучить собственную модель на датасете WIDERFace командой `python -m backend.utils.train_face_detector` (подробнее ниже) и взять итоговый файл из `backend/weights`.
...
- Скрипт автоматически скачает и конвертирует датасет в формат YOLO, запустит обучение и положит лучшие веса в `backend/weights/yolo11n-face.pt`. При повторных запусках используйте флаг `--skip-download`, чтобы не скачивать архивы заново.
