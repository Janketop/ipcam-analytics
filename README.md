...
- `FACE_DETECTOR_WEIGHTS` — путь до весов, если используется YOLO-вариант. По умолчанию берётся значение из `YOLO_FACE_MODEL` (`weights/yolov11n-face.pt`). Файл можно получить двумя способами: (1) скачать готовую модель YOLOv11n Face Detection из релизов репозитория [YapaLab/yolo-face](https://github.com/YapaLab/yolo-face) (бэкенд автоматически тянет `yolov11n-face.pt` при первом запуске); (2) обучить собственную модель на датасете WIDERFace командой `python -m backend.utils.train_face_detector` (подробнее ниже) и указать путь к полученному файлу вручную.
