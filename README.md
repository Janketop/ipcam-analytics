...
- `FACE_DETECTOR_TYPE` — тип специализированного детектора лиц. Доступны значения `onnx` (используется по умолчанию), `yolo` и `mediapipe`. При выборе `mediapipe` понадобится установить одноимённую библиотеку в образ backend (`pip install mediapipe`). Если модель недоступна, сервис автоматически откатится к рамке тела, поэтому поток не остановится.
- `FACE_DETECTOR_WEIGHTS` — вручную заданный путь до весов. Если переменная не указана, сервис сначала попробует `ONNX_FACE_MODEL`, когда включён ONNX-бэкенд или выбран тип `FACE_DETECTOR_TYPE=onnx`, и только затем вернётся к `YOLO_FACE_MODEL` (PyTorch). Базовые файлы можно получить двумя способами: (1) скачать готовые модели из релизов репозитория [YapaLab/yolo-face](https://github.com/YapaLab/yolo-face); (2) обучить собственную модель на датасете WIDERFace командой `python -m backend.utils.train_face_detector` (подробнее ниже) и указать путь к полученному файлу вручную.
...
```
Полученные файлы положите в `backend/models/weights` и пропишите соответствующие переменные окружения. Для классификатора лиц можно оставить заглушку `backend/models/weights/face_recognition_arcface_dummy.onnx` — при первом запуске сервис автоматически скачает полноценные веса ArcFace (InsightFace) и заменит файл. Источники по умолчанию соответствуют официальным архивам [deepinsight/insightface](https://github.com/deepinsight/insightface) (`buffalo_l.zip`, `buffalo_s.zip`, `buffalo_sc.zip`, `antelopev2.zip`) и зеркалам на HuggingFace. Если требуется вручную указать зеркала или локальные файлы, используйте переменную `ARCFACE_WEIGHTS_SOURCES` (разделяйте значения запятой/переносом строки) — путь до каждой модели можно подсмотреть в релизах InsightFace. При желании можно заранее подложить собственные эмбеддинги. Сервис сам превратит относительные пути в абсолютные, что упрощает вынос моделей в отдельные тома.

Контейнер backend собирается на базе `nvidia/cuda:13.0.0-base-ubuntu22.04` (драйверы серии 581.80 и CUDA 13.0), устанавливает `onnxruntime-gpu`, поэтому дополнительные зависимости ставить не нужно. В `docker-compose.yml` уже прописано пробрасывание GPU (`deploy.resources.reservations.devices` + переменные `NVIDIA_VISIBLE_DEVICES`), достаточно запустить `docker compose up --build` на машине с установленным NVIDIA Container Toolkit.
