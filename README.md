...
- `YOLO_DEVICE` — устройство для инференса (`cpu`, `cuda:0` и т.д.). Если устройство недоступно, модели продолжат работать на значении по умолчанию. Для полноценного ускорения на GPU достаточно выставить `YOLO_DEVICE=cuda:0` (и убедиться, что контейнер запущен с доступом к GPU).
