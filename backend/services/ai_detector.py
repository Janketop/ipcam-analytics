...
class AIDetector:
    ...
    def __init__(
        ...
        *,
        ...
        face_weights = (settings.yolo_face_model or "").strip()
        ...
        self.face_det_error: Optional[str] = None
        if face_weights:
            try:
                self.face_det = YOLO(face_weights)
            except Exception as exc:
                self.face_det_error = str(exc).strip() or exc.__class__.__name__
                logger.warning(
                    "[%s] Не удалось загрузить веса YOLOv8-face (%s): %s",
                    self.camera_name,
                    face_weights,
                    self.face_det_error,
                )
        else:
            self.face_det_error = "путь до весов не указан"
    ...
    def process_frame(
        ...
        if detect_person and self.face_det is not None:
            ...
            try:
                ...
                if not self._face_warned:
                    logger.warning(
                        "[%s] Ошибка инференса YOLOv8-face: %s",
                        self.camera_name,
                        self.face_det_error,
                    )
        ...
        elif detect_person and self.face_det is None and not self._face_warned:
            logger.warning(
                "[%s] Детектор лиц YOLO отключён: %s",
                self.camera_name,
                self.face_det_error or "неизвестная причина",
            )
            self._face_warned = True
